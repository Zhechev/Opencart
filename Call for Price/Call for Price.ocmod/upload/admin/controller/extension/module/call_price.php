 ...
 * @CID       $ClientID$
*/
class ControllerExtensionModuleCallPrice extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('extension/module/call_price');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('module_call_price', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/call_price', 'user_token=' . $this->session->data['user_token'], true)
		);

		$data['action'] = $this->url->link('extension/module/call_price', 'user_token=' . $this->session->data['user_token'], true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);

		if (isset($this->request->post['module_call_price_status'])) {
			$data['call_price_status'] = $this->request->post['module_call_price_status'];
		} else {
			$data['call_price_status'] = $this->config->get('module_call_price_status');
		}
		...
		$this->response->setOutput($this->load->view('extension/module/call_price', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/call_price')) {
	...
	public function install() {
		$this->load->model('extension/module/call_price');
		
		$this->model_extension_module_call_price->install();
	}

	public function uninstall() {
		$this->load->model('extension/module/call_price');
		
		$this->model_extension_module_call_price->uninstall();
	}
}